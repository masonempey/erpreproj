...
export default function Admin() {
  ...
  useEffect(() => {
    const appointmentsSelected = appointments
    .filter((appointment) => dayjs(appointment.startTime).isSame(selectedDate, "day"));
  ...
  const appointmentDays = appointments.map(appointment => dayjs(appointment.startTime));

  return (
    <main>
      <AdminNavBar handleChangeView={handleChangeView} selectedView={selectedView} tutorialDisplay={tutorialDisplay} handleTutorialDisplay={handleTutorialDisplay} />
            ...
            {selectedAppointment ? (
              <div className={styles.appointmentDetails}>
                <div className={styles.appointmentButtons}>
                  <ArrowBackIcon onClick={() => setSelectedAppointment(null)} sx={{cursor: "pointer", fontSize: 50, bgcolor: grey[300], borderRadius: "20%", width:80, mb:2}}/>
                  <DoDisturbOnIcon onClick={() => setSelectedAppointment(null)} sx={{cursor: "pointer", fontSize: 50, bgcolor: grey[300], borderRadius: "20%", width:80, mb:2, color: red[500]}}/>
                </div>
                <div className={styles.appointmentHeader}>
                  <h2>Appointment Details</h2>
                  <Avatar src="/broken-image.jpg" sx={{width: 120, height: 120, m: 3}}/>
                </div>
                <div className={styles.appointmentInfo}>
                  <p><span>Customer:</span> {selectedAppointment.customerName}</p>
                  <p><span>Phone Number:</span> {selectedAppointment.phone}</p>
                </div>
                <div className={styles.appointmentInfo}>
                  <p><span>Service:</span> {selectedAppointment.serviceType}</p>
                  <p><span>Email:</span> <a href={`$mailto:{selectedAppointment.email}`}>{selectedAppointment.email}</a></p>
                </div>
                <p className={styles.appointmentDate}><span>Appointment Time:</span> {formatTime(selectedAppointment.startTime)} - {formatTime(selectedAppointment.endTime)}</p>
              </div>
            ) : (
              <TimeSlot appointmentsThisDay={appointmentsThisDay} handleViewAppointment={handleViewAppointment}/>
            )}
          </div>
        </section>  
      </div>

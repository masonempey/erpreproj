...
export async function saveReview(reviewData) {
  try {
    ...
    // This is a convenient way to access properties like reviewId, reviewDate, etc.
    const { reviewId, reviewDate, review, userId, rating, profilePhotoUrl } = reviewData;
    ...
    // pool.query with an INSERT statement adds a new row to the reviews table
    // $1, $2, ..., $6 are placeholders for the values in the array
    // RETURNING * tells PostgreSQL to return the newly inserted row
    const result = await pool.query(
      `INSERT INTO reviews (review_id, review_date, review, user_id, rating, profile_photo_url)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [reviewId, reviewDate, review, userId, rating, profilePhotoUrl]
    );
